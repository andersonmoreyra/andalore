...
function NewWOModal() {
  ...
  React.useEffect(() => {
    const onOpen = (e) => {
      ...
      if (e.detail) {
        ...
        setForm({
          titulo: "", equipamento: "", tipo: "corretiva", prioridade: "média",
          tecnico: "", estimativa: 60, descricao: "",
          solicitante: "Coord. F. Almeida", attachments: [],
        });
      }
    };
  ...
  const valid1 = form.titulo.trim() && form.equipamento && form.tipo && form.prioridade;
  ...
  const submit = () => {
    ...
    const tech = technicians.find(t => t.id === form.tecnico);
    ...
    const newWO = {
      ...
      equipamentoNome: asset?.nome || form.equipamento,
      area: asset?.area || 'N/A',
      tipo: form.tipo,
      prioridade: form.prioridade,
      tecnico: form.tecnico || null,
      tecnicoNome: tech?.nome || null,
      estimativa: form.estimativa,
            ...
            <>
              ...
              <div className="form-grid">
                <div className="form-row">
                  ...
                  <select className="form-input" value={form.equipamento} onChange={(e) => set("asset", e.target.value)}>
                  ...
                  <div className="form-segments">
                    {[
                      ...
                      <button key={o.v} type="button"
                              data-no-toast="true"
                              className="form-seg"
                              data-active={form.prioridade === o.v}
                              data-tone={o.tone}
                ...
                <div className="form-row">
                  ...
                  <div className="form-time-input">
                    <input type="number" min="5" step="5" className="form-input"
                           ...
                           onChange={(e) => set("estimativa", parseInt(e.target.value) || 0)} />
                    ...
                    <div className="form-time-presets">
                      {[30, 60, 120, 240].map(p => (
                        <button key={p} type="button" data-no-toast="true"
                                ...
                                data-active={form.estimativa === p}
                                onClick={() => set("estimativa", p)}>{p < 60 ? `${p}m` : `${p/60}h`}</button>
                      ))}
                    </div>
                  </div>
                </div>
            ...
            <>
              <div className="form-summary">
                ...
                <div className="form-summary-row">
                  ...
                  <span className="form-summary-v"><strong>{form.titulo}</strong></span>
                </div>
                <div className="form-summary-row">
                  <span className="form-summary-k">Ativo</span>
                  <span className="form-summary-v mono">{form.equipamento || "—"}</span>
                </div>
                <div className="form-summary-row">
                  <span className="form-summary-k">Tipo / Prioridade</span>
                  <span className="form-summary-v">
                    <span className="pill" data-tone={toneType(form.tipo)}>{form.tipo}</span>
                    <span className="pill" data-tone={tonePriority(form.prioridade)} style={{ marginLeft: 6 }}>{form.prioridade}</span>
                  </span>
                </div>
                <div className="form-summary-row">
                  <span className="form-summary-k">Tempo estimado</span>
                  <span className="form-summary-v mono">{fmtMin(form.estimativa)}</span>
                </div>
              </div>
                  ...
                  {technicians.map(t => (
                    <button key={t.id} type="button"
                      ...
                      <span className="assignee-pick-state">
                        {form.tecnico === t.id ? "✓ Selecionado" : "Selecionar"}
                      </span>
                    </button>
                  ))}
                </div>
                <div className="form-hint">
                  Você pode deixar não atribuído e fazer a triagem depois — a OS entra na fila com prioridade {form.prioridade}.
                </div>
              </div>
