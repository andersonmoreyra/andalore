...
function ShiftTimeline() {
  ...
  const blocks = {
    T01: [{ s: 6, e: 9.5, wo: "OS-2841", titulo: "Subst. cilindro Flexo 1", tone: "info" }],
    T02: [{ s: 8.25, e: 9.25, wo: "OS-2847", titulo: "Calibração CP-01", tone: "pred" },
          { s: 10, e: 12, wo: "—", titulo: "Janela livre", tone: "ghost" }],
    T03: [{ s: 6.2, e: 8.4, wo: "OS-2839", titulo: "Vibração eixo EX-01", tone: "crit" },
          { s: 8.5, e: 11, wo: "OS-2832", titulo: "Vazamento CT-02", tone: "warn" }],
    T04: [{ s: 6, e: 7, wo: "—", titulo: "Reunião turno", tone: "ghost" },
          { s: 7, e: 14, wo: "—", titulo: "Disponível", tone: "free" }],
    T05: [{ s: 6, e: 9, wo: "—", titulo: "Folga programada", tone: "ghost" },
          { s: 9, e: 9.5, wo: "—", titulo: "Pausa", tone: "ghost" }],
    T06: [{ s: 7.8, e: 10, wo: "OS-2845", titulo: "Curto painel EX-02", tone: "crit" }],
    T07: [{ s: 7, e: 8.5, wo: "OS-2842", titulo: "Lubrif. Cortadeira 01", tone: "info" }],
    T08: [{ s: 6, e: 14, wo: "—", titulo: "Disponível", tone: "free" }],
  };
          ...
          <div key={t.id} className="tl-row">
            <div className="tl-tech-cell">
              ...
              <div className="tl-tech-info">
                <div className="tl-tech-name">{t.nome}</div>
            ...
            <div className="tl-track">
              ...
              {(blocks[t.id] || []).map((b, i) => (
                <div key={i} className={`tl-block tl-${b.tone}`}
                  ...
                  <span className="tl-block-title">{b.titulo}</span>
                </div>
              ))}
            </div>
          </div>
...
function CriticalAlerts({ onOpenWO }) {
  const critical = WORK_ORDERS.filter(w => w.prioridade === "crítica" || (w.prioridade === "alta" && w.sla > 0.7));
  return (
        ...
        {critical.map((w) => (
          <button key={w.id} className="alert-row" onClick={() => onOpenWO(w.id)}>
            <div className="alert-row-l">
              <span className="pill" data-tone={tonePriority(w.prioridade)}>
                <span className="dot" />{w.prioridade}
              </span>
              <span className="mono alert-id">{w.id}</span>
              <span className="alert-title">{w.titulo}</span>
            </div>
            <div className="alert-row-r">
              <span className="alert-asset mono">{w.equipamento}</span>
              ...
              {ASSETS.filter(a => a.area === area).map((a) => {
                ...
                return (
                  <div key={a.id} className="health-cell" data-tone={tone}
                       data-crit={a.criticidade}
                       title={`${a.nome} · saúde ${Math.round(a.saude*100)}% · MTBF ${a.mtbf}h`}>
                    <span className="health-cell-id mono">{a.id}</span>
                    <span className="health-cell-bar">
                      <i style={{ width: `${a.saude * 100}%` }} />
                    </span>
                    <span className="health-cell-crit">{a.criticidade}</span>
                  </div>
                );
              })}
            </div>
        ...
        {REQUESTS.map((r) => (
          <div key={r.id} className="request-row">
            <div className="request-head">
              ...
              <span className="pill" data-tone={tonePriority(r.prioridade)}>{r.prioridade}</span>
              <span className="mono request-time">{r.time}</span>
            </div>
            <div className="request-title">{r.titulo}</div>
            <div className="request-meta">
              <span>{r.from}</span>
              <span>·</span>
              <span className="mono">{r.equipamento}</span>
